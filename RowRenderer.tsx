...
const ShopeeDealsTopUpsBills = ({data}) => {
  const SLIDE_HEIGHT = 100;
  const SLIDE_WIDTH = width - 10 * 2;
  const CAT_HEIGHT = 80;
  const CAT_WIDTH = 70;
...
const ShopeeTrendingSearch = ({data}) => {
...
const ShopeeFeaturedCollection = ({data}) => {
      ...
      <Pressable
        ...
        <View>
          ...
          <View style={{padding: 6, flex: 1, justifyContent: 'space-between'}}>
            ...
            <Text style={{fontSize: 12, color: '#9CA3AF'}}>{item.caption}</Text>
          </View>
        </View>
      </Pressable>
    );
...
const ShopeeCategorySelector = ({data}) => {
  const PADDING = 4;
